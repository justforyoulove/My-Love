...
import { SecretScratchModal } from './components/SecretScratchModal';
...
export default function App() {
  const [customization, setCustomization] = useState<AppCustomization>(() => {
    ...
    if (saved) {
      try {
        ...
        if (parsed.scratchCards) {
          delete parsed.scratchCards;
        }
        // Ensure updated default reasons and names take precedence if needed
        const mergedReasons = (!parsed.reasons || parsed.reasons.length < DEFAULT_CUSTOMIZATION.reasons.length)
          ? DEFAULT_CUSTOMIZATION.reasons
          : parsed.reasons;
        return { ...DEFAULT_CUSTOMIZATION, ...parsed, reasons: mergedReasons };
      } catch (e) {
  ...
  const [showSecretScratch, setShowSecretScratch] = useState(false);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-rose-100 font-sans selection:bg-rose-500 selection:text-white relative overflow-x-hidden pb-24">
      ...
      {/* Footer */}
      <footer className="text-center py-8 text-xs text-rose-400/60 space-y-1.5 font-serif select-none">
        <p>
          Created with endless{' '}
          <button
            onClick={() => setShowSecretScratch(true)}
            className="text-rose-300 hover:text-white underline decoration-rose-500/50 hover:decoration-rose-300 font-bold cursor-pointer transition-colors px-1 py-0.5 rounded bg-rose-500/10 hover:bg-rose-500/20 inline-flex items-center gap-1 group shadow-sm"
            title="✨ Click to reveal hidden secret scratch card!"
          >
            <span>love</span>
            <Sparkles className="w-3 h-3 text-rose-400 group-hover:scale-125 transition-transform inline" />
          </button>{' '}
          by {customization.boyfriendName}
        </p>
        <p className="text-[10px] text-rose-500/40">Forever & Always • Girlfriend's Day Special</p>
      </footer>

      {/* 10. Hidden Easter Egg Secret Scratch Card Modal */}
      <SecretScratchModal
        isOpen={showSecretScratch}
        onClose={() => setShowSecretScratch(false)}
        girlfriendName={customization.girlfriendName}
        boyfriendName={customization.boyfriendName}
        onTriggerHeartBurst={fireHeartBurst}
      />
